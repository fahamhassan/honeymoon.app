    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-8 bg-white border border-[rgba(184,154,105,0.2)] rounded-lg shadow-lg z-20 min-w-[140px] py-1 overflow-hidden">
            {items.map((item, i) => (
              <button key={i} onClick={() => { item.onClick(); setOpen(false); }}
                className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-sm font-medium hover:bg-[#f4ebd0] text-left transition-colors ${item.danger ? 'text-red-600' : 'text-[#1a1a1a]'}`}>
  ...
  return (
    <div className="flex items-center justify-between mb-4">
      <div className="flex items-center gap-2">
        ...
        <div className="bg-[#f1ebe1] border border-[rgba(184,154,105,0.2)] rounded-md px-3 py-1.5 flex items-center gap-1 cursor-pointer">
      ...
      <div className="flex items-center gap-2">
        {onFilter && (
          <button onClick={onFilter} className="bg-[#f1ebe1] border border-[rgba(184,154,105,0.2)] rounded-md p-2 hover:bg-[#e8e0d0] transition-colors">
        ...
        <div className="relative">
          <input value={search} onChange={e => onSearch(e.target.value)} placeholder="Search..."
            className="bg-[#f1ebe1] border border-[rgba(184,154,105,0.2)] rounded-md pl-9 pr-4 py-2 text-sm text-[#1a1a1a] outline-none focus:border-[#B8965A] w-64 placeholder:text-[#999]"/>
