  ...
  return (
    <div className="min-h-screen flex items-center justify-center" style={{background:'#f4ebd0'}}>
      <div className="flex items-center gap-10 max-w-5xl w-full px-8">
        {/* Card */}
        <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-10 w-full max-w-[440px] flex-shrink-0">
          ...
          {/* Tabs */}
          <div className="flex border border-[rgba(184,154,105,0.3)] rounded-xl overflow-hidden mb-6">
            {tabs.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`flex-1 py-2.5 text-xs font-medium transition-colors ${tab===t.id?'bg-[#174a37] text-white':'text-gray-500 hover:bg-[#f4ebd0]'}`}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          {tab === 'mobile' && (
            <div className="mb-4">
              ...
              <div className="flex gap-2">
                <div className="flex items-center gap-1 border border-gray-200 rounded-xl px-3 py-3 flex-shrink-0 bg-[#faf8f4]">
                ...
                <input placeholder="Enter Mobile Number" value={form.mobile} onChange={e=>setForm(p=>({...p,mobile:e.target.value}))}
                  className="flex-1 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#b89b6b] bg-[#faf8f4]" />
              </div>
            </div>
          )}
          ...
          <button onClick={handleLogin} disabled={loading}
            className="block w-full bg-[#b89b6b] text-white text-center font-medium py-3.5 rounded-full hover:bg-[#a08860] transition-colors disabled:opacity-60">
