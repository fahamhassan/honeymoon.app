    ...
    <div>
      ...
      <div className="bg-white rounded-2xl shadow-[0_0_20px_rgba(0,0,0,0.05)] overflow-hidden max-w-4xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          ...
          <button onClick={() => setNotifs(p => p.map(n => ({...n, read:true})))} className="text-sm font-medium text-[#b89b6b] hover:underline">Mark All As Read</button>
        </div>
        <div className="divide-y divide-gray-50">
          {shown.map(n => (
            <div key={n.id} className={`flex items-start justify-between gap-4 px-6 py-5 hover:bg-[#fafaf8] transition-colors ${!n.read?'bg-[#fdfaf4]':''}`}>
              ...
              <button onClick={() => setNotifs(p => p.map(x => x.id===n.id ? {...x,read:!x.read} : x))}
                className="text-sm text-[#b89b6b] hover:text-[#174a37] transition-colors whitespace-nowrap font-medium">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100">
          ...
          <div className="flex items-center gap-2">
            <button className="px-4 py-2 text-sm border border-gray-200 rounded-xl hover:bg-[#f4ebd0] opacity-40 cursor-not-allowed">Previous</button>
            {[1,2].map(p => <button key={p} className={`w-9 h-9 rounded-xl text-sm ${p===1?'bg-[#174a37] text-white':'text-gray-500 hover:bg-[#f4ebd0]'} transition-colors`}>{p}</button>)}
