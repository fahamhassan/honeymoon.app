        ...
        {STEPS.map((s,i) => (
          <div key={s} className="flex-1 flex items-center cursor-pointer" onClick={() => setStep(i+1)}>
            <div className="flex items-center gap-1.5">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-colors ${i+1===step?'bg-[#174a37] text-white border-[#174a37]':i+1<step?'bg-[#b89b6b] text-white border-[#b89b6b]':'bg-white text-gray-400 border-gray-200'}`}>
                {i+1<step?'✓':i+1}
              </div>
      ...
      <div className="bg-white rounded-2xl p-8 shadow-[0_0_20px_rgba(0,0,0,0.05)]">
        {step===1 && (
          <div className="flex flex-col gap-4">
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Service Description<span className="text-red-500">*</span></label>
              <textarea value={form.desc} onChange={e=>f('desc',e.target.value)} rows={5} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#b89b6b] bg-[#faf8f4] resize-none"/></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Location<span className="text-red-500">*</span></label>
              <input value={form.location} onChange={e=>f('location',e.target.value)} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#b89b6b] bg-[#faf8f4]"/></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-2">Upload Image<span className="text-red-500">*</span></label>
              <div className="flex gap-3">
                ...
                <div className="w-32 h-24 border-2 border-dashed border-[rgba(184,154,105,0.3)] rounded-xl flex items-center justify-center cursor-pointer hover:border-[#b89b6b] transition-colors">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Deposit (%)</label>
                <input value={form.deposit} onChange={e=>f('deposit',e.target.value)} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#b89b6b] bg-[#faf8f4]"/></div>
            </div>
          </div>
        )}
        {step===3 && (
            ...
            {form.addons.map((a,i)=>(
              <div key={i} className="flex gap-3 mb-3 items-end">
                <div className="flex-1"><label className="block text-xs text-gray-500 mb-1">Add-On Name</label>
                  <input value={a.name} onChange={e=>setForm(p=>({...p,addons:p.addons.map((x,j)=>j===i?{...x,name:e.target.value}:x)}))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#b89b6b] bg-[#faf8f4]"/></div>
                <div className="w-28"><label className="block text-xs text-gray-500 mb-1">Price (AED)</label>
                  <input value={a.price} onChange={e=>setForm(p=>({...p,addons:p.addons.map((x,j)=>j===i?{...x,price:e.target.value}:x)}))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#b89b6b] bg-[#faf8f4]"/></div>
          ...
          <div className="flex flex-col gap-4">
            {[['Cancellation Policy','cancelPolicy'],['Terms & Conditions','terms'],['Refund Policy','refund']].map(([l,k])=>(
              <div key={k}><label className="block text-sm font-medium text-gray-700 mb-1">{l}</label>
                <textarea value={form[k]} onChange={e=>f(k,e.target.value)} rows={3} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#b89b6b] bg-[#faf8f4] resize-none"/></div>
            ))}
          </div>
        )}
        ...
        <div className="flex gap-3 mt-6">
          ...
          {step<4 ?
            ...
            <button onClick={()=>setSuccess(true)} className="flex-1 bg-[#b89b6b] text-white py-3 rounded-full font-medium hover:bg-[#a08860] transition-colors">Save Changes ↗</button>
          }
        </div>
      </div>
    </div>
