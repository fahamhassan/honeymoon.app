  ...
  return (
    <button onClick={onClick} disabled={disabled}
      className={`min-w-[32px] h-8 rounded-full border-none text-sm px-1 transition-all
        ...
        ${disabled ? 'text-[#C0B8A8] cursor-default' : active ? '' : 'text-[#1A1A1A] cursor-pointer hover:bg-[#f4ebd0]'}
      `}
    >{label}</button>
  );
}
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(v => !v)}
        className="bg-transparent border-none cursor-pointer text-xl text-[#6B6050] px-2 py-1 rounded-md hover:bg-[#f4ebd0] transition-colors">⋮</button>
      {open && (
        <>
          <div onClick={() => setOpen(false)} className="fixed inset-0 z-10" />
          <div className="absolute right-0 top-full bg-white rounded-xl shadow-xl min-w-[130px] z-20 overflow-hidden border border-[rgba(184,154,105,0.15)]">
