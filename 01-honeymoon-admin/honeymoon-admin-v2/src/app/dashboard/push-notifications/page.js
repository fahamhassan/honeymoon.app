...
export default function PushNotificationsPage() {
  useApi(AdminService.getPushNotifications);
  const [rows, setRows] = useState(NOTIFS);
  const [confirm, setConfirm] = useState(null);
  const [success, setSuccess] = useState('');

  function doDelete() {
    setRows(p => p.filter(n => n.id!==confirm.id));
    setConfirm(null); setSuccess('Notification deleted successfully.');
  }
              ...
              <tr key={n.id} className="border-b border-gray-50 hover:bg-[#fafaf8] transition-colors">
                ...
                <td className="py-3.5 px-4">
                  <div className="flex gap-1.5">
                    <Link href={`/dashboard/push-notifications/${n.id}`} className="text-xs text-[#174a37] border border-[rgba(23,74,55,0.3)] px-2.5 py-1.5 rounded-lg hover:bg-[#f4ebd0] transition-colors">View</Link>
