  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{background:'rgba(0,0,0,0.6)'}}>
      <div className="bg-white rounded-2xl shadow-2xl p-5 sm:p-8 w-full max-w-[520px] relative">
        ...
        <h3 className="font-baskerville text-2xl text-[#b89b6b] mb-6">Edit — {section.name}</h3>
        <div className="flex flex-col gap-4">
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Title</label>
            <input value={form.title} onChange={e=>setForm(p=>({...p,title:e.target.value}))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#b89b6b] bg-[#faf8f4]" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Subtitle</label>
            <textarea value={form.subtitle} onChange={e=>setForm(p=>({...p,subtitle:e.target.value}))} rows={3} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#b89b6b] bg-[#faf8f4] resize-none" /></div>
          {form.btnText !== undefined && <div><label className="block text-sm font-medium text-gray-700 mb-1">Button Text</label>
            <input value={form.btnText} onChange={e=>setForm(p=>({...p,btnText:e.target.value}))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#b89b6b] bg-[#faf8f4]" /></div>}
...
export default function HomeContentPage() {
  ...
  function saveSection(form) {
    setSections(p => p.map(s => s.id===editing.id ? {...s,...form} : s));
    setEditing(null);
  ...
  return (
    <div>
      ...
      <h1 className="font-baskerville text-3xl text-[#1a1a1a] mb-6">Home Content Management</h1>
      <div className="flex flex-col gap-4">
        {sections.map(s => (
          <div key={s.id} className="bg-white rounded-2xl p-6 shadow-[0_0_20px_rgba(0,0,0,0.05)] flex items-center gap-6">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[#b89b6b] text-xs uppercase tracking-wider font-medium">{s.name}</p>
              <h3 className="font-medium text-[#1a1a1a] mt-0.5 truncate">{s.title}</h3>
              <p className="text-gray-400 text-sm mt-0.5 truncate">{s.subtitle}</p>
            </div>
            <button onClick={() => setEditing(s)} className="bg-[#174a37] text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-[#1a5c45] transition-colors flex-shrink-0">Edit</button>
          </div>
        ))}
      </div>
    </div>
