...
const REDEMPTION_LOGS = Array.from({length:10},(_,i)=>({
  id:String(i+1).padStart(2,'0'), user:'User ABC', points:500, value:'50 AED', date:'07/05/2022', status:i%3===0?'Pending':'Redeemed'
}));
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex rounded-full border border-[rgba(23,74,55,0.3)] overflow-hidden">
          <button onClick={() => setTab('award')} className={`px-6 py-2.5 text-sm font-medium transition-colors ${tab==='award'?'bg-[#174a37] text-white':'text-[#174a37] hover:bg-[#f4ebd0]'}`}>Award</button>
          <button onClick={() => setTab('redemption')} className={`px-6 py-2.5 text-sm font-medium transition-colors ${tab==='redemption'?'bg-[#174a37] text-white':'text-[#174a37] hover:bg-[#f4ebd0]'}`}>Redemption</button>
        </div>
      </div>
