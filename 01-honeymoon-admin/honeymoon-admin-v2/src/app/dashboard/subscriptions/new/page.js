  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link href="/dashboard/subscriptions" className="text-gray-400 hover:text-gray-600 text-lg">←</Link>
        <h1 className="font-baskerville text-3xl text-[#1a1a1a]">New Subscription</h1>
      </div>
      <div className="bg-white rounded-2xl p-8 shadow-[0_0_20px_rgba(0,0,0,0.05)]">
        <div className="flex flex-col gap-4">
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Subscription Name<span className="text-red-500">*</span></label>
            <input value={form.name} onChange={e=>f('name',e.target.value)} placeholder="e.g. Premium Plan"
              className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#b89b6b] bg-[#faf8f4]"/></div>
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={()=>form.name&&form.priceMonthly&&setSuccess(true)}
            className="flex-1 bg-[#174a37] text-white py-3.5 rounded-full font-medium hover:bg-[#1a5c45] transition-colors">
            Create Plan ↗
          </button>
          <Link href="/dashboard/subscriptions" className="flex-1 border border-gray-200 text-gray-500 py-3.5 rounded-full font-medium hover:bg-gray-50 transition-colors text-center">
            Cancel
          </Link>
        </div>
      </div>
