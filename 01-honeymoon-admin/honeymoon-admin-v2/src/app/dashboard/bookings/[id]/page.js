      ...
      <div className="bg-white rounded-2xl p-6 shadow-[0_0_20px_rgba(0,0,0,0.05)]">
        ...
        {(status==='Pending'||status==='Upcoming')&&(
          <div className="flex gap-3 mt-6 pt-5 border-t border-gray-100">
            ...
            {status==='Pending'&&<button onClick={()=>setModal('cancel')} className="border border-red-300 text-red-500 px-6 py-2.5 rounded-full text-sm font-medium hover:bg-red-50 transition-colors">Cancel Booking</button>}
          </div>
        )}
      </div>
    </div>
