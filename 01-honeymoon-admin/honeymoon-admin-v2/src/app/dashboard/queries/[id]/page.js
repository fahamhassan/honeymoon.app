  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="flex items-center gap-2 mb-6 text-sm text-gray-400">
        <Link href="/dashboard/queries" className="hover:text-[#174a37]">Queries</Link>
        <span>/</span><span className="text-gray-800">Query #{params.id}</span>
      </div>
      <div className="bg-white rounded-2xl p-8 shadow-[0_0_20px_rgba(0,0,0,0.05)]">
        <div className="flex items-center justify-between mb-5">
          <h1 className="font-baskerville text-2xl text-[#1a1a1a]">Query Details</h1>
          <span className={`text-xs px-3 py-1.5 rounded-full font-medium ${status==='Resolved'?'text-green-700 bg-green-100':status==='In Progress'?'text-amber-700 bg-amber-100':'text-blue-700 bg-blue-100'}`}>{status}</span>
        </div>
        ...
        {status !== 'Resolved' && (
          <>
            ...
            <div className="flex gap-3">
              <button onClick={sendReply} className="flex-1 bg-[#174a37] text-white py-3 rounded-full text-sm font-medium hover:bg-[#1a5c45] transition-colors">Send Reply</button>
              <button onClick={resolve} className="flex-1 bg-green-50 text-green-600 border border-green-200 py-3 rounded-full text-sm font-medium hover:bg-green-100 transition-colors">Mark Resolved</button>
            </div>
          </>
        )}
        {status === 'Resolved' && (
