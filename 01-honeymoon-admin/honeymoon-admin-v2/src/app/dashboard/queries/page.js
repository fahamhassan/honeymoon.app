...
function ViewQueryModal({ query, onClose, onReply, onResolve }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{background:'rgba(0,0,0,0.6)'}}>
      <div className="bg-white rounded-2xl shadow-2xl p-5 sm:p-8 w-full max-w-[580px] relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-5">
          <h3 className="font-baskerville text-2xl text-[#1a1a1a]">Query #{query.id}</h3>
          <span className={`text-xs px-3 py-1 rounded-full font-medium ${query.status==='Resolved'?'text-green-700 bg-green-100':query.status==='In Progress'?'text-amber-700 bg-amber-100':'text-blue-700 bg-blue-100'}`}>{query.status}</span>
        </div>
        ...
        {query.status !== 'Resolved' && (
          <>
            ...
            <div className="flex gap-3">
              <button onClick={() => onReply(reply)} className="flex-1 bg-[#174a37] text-white py-3 rounded-full text-sm font-medium hover:bg-[#1a5c45] transition-colors">Send Reply</button>
              <button onClick={onResolve} className="flex-1 bg-green-50 text-green-600 border border-green-200 py-3 rounded-full text-sm font-medium hover:bg-green-100 transition-colors">Mark Resolved</button>
            </div>
          </>
        )}
        {query.status === 'Resolved' && (
          <button onClick={onClose} className="w-full border border-gray-200 text-gray-500 py-3 rounded-full text-sm font-medium hover:bg-gray-50 transition-colors">Close</button>
        )}
      </div>
    </div>
  );
...
export default function QueriesPage() {
  ...
  return (
    ...
    <div>
      ...
      <h1 className="font-baskerville text-3xl text-[#1a1a1a] mb-6">Queries</h1>
            ...
            {data.map(q => (
              <tr key={q.id} className="border-b border-gray-50 hover:bg-[#fafaf8] transition-colors">
                ...
                <td className="py-3.5 px-4">
                  <button onClick={() => setViewing(q)} className="text-xs text-[#174a37] border border-[rgba(23,74,55,0.3)] px-3 py-1.5 rounded-lg hover:bg-[#f4ebd0] transition-colors">View</button>
                </td>
              </tr>
            ))}
          </tbody>
