...
const imgGlobe = `data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="%23174a37" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>`
const imgEllipse = `data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><ellipse cx="50" cy="50" rx="50" ry="50" fill="%23174a37" opacity="0.1"/></svg>`
...
const imgFacebook = `data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%231877F2"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>`
const imgFacebook1 = `data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%231877F2"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>`
const imgArrowRight = `data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="%23174a37" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>`
...
export function PublicNavbar({ activePage = 'Home', dark = false }) {
  ...
  return (
    <nav className="w-full fixed top-0 left-0 z-50" style={{ background: 'rgba(23,74,55,0.95)', backdropFilter: 'blur(8px)', height: '112px' }}>
      <div className="max-w-[1440px] mx-auto px-4 sm:px-10 h-full flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-8">
          {NAV_ITEMS.map(item => (
            <Link
              ...
              className="text-white text-[14px] uppercase tracking-wide hover:text-[#b89b6b] transition-colors"
        ...
        <div className="flex items-center gap-6">
          ...
          <Link href="/login" className="relative overflow-hidden rounded-[7.5px] px-5 py-2.5 text-white text-[14px] uppercase border border-white/30 hover:border-[#b89b6b] transition-colors whitespace-nowrap">
            Sign Up | Login
          </Link>
        </div>
      </div>
...
export function LoggedInNavbar({ user = { name: 'Rashed Kabir' } }) {
  ...
  return (
    <nav className="w-full fixed top-0 left-0 z-50" style={{ background: 'rgba(23,74,55,0.97)', backdropFilter: 'blur(8px)', height: '112px' }}>
      <div className="max-w-[1440px] mx-auto px-4 sm:px-10 h-full flex items-center justify-between">
        {/* Logo */}
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-[#b89b6b] flex items-center justify-center text-white font-bold text-sm">
              {user.name?.charAt(0)}
            </div>
          ...
          <div className="flex flex-col gap-8 min-w-0 md:min-w-[500px]">
            ...
            <div className="flex gap-6 flex-wrap">
              {['Home', 'About', 'Vendors', 'Services', 'Contact'].map((item, i) => (
                <span key={item}>
                  <Link href={`/${item.toLowerCase()}`} className={`text-[18px] ${i === 0 ? 'text-white' : 'text-white/30'} hover:text-[#b89b6b] transition-colors`}>
                    {item}
                  </Link>
            ...
            <div className="flex items-center gap-4">
              {[
                ...
                <button key={label} className="relative w-12 h-12 flex items-center justify-center cursor-pointer">
...
export function SmartAssistantBadge() {
  const imgBadge = `data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%23b89b6b"><text y="20" font-size="20">🏆</text></svg>`
  const imgIcon = `data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%23174a37"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>`
  return (
    <div className="fixed bottom-10 right-10 z-40 flex flex-col items-center gap-2 cursor-pointer">
      <div className="relative w-[89px] h-[89px]">
        <img src={imgBadge} alt="" className="w-full h-full object-contain" />
        <img src={imgIcon} alt="" className="absolute inset-0 m-auto w-[65px] h-[69px] object-contain" />
      </div>
