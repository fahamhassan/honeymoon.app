...
const footerLogoIcon = "/logo-icon.png";
const footerLogoText = "/logo-text.png";
const footerArabic = "/logo-arabic.png";

export default function Footer() {
  const [email, setEmail] = useState('');
  return (
    <footer className="bg-[#1a1a1a] py-12">
      <div className="max-w-7xl mx-auto px-8 flex flex-col md:flex-row gap-10 justify-between">
        <div>
        ...
        <div className="flex-1 max-w-[500px]">
          ...
          <div className="flex items-center gap-4 mb-5">
            {['X','Fb','In','Li'].map(s => (
              <button key={s} className="w-10 h-10 border border-white/20 rounded-full flex items-center justify-center text-white/60 hover:text-white hover:border-white/40 text-sm font-medium transition-colors">{s}</button>
            ))}
          </div>
