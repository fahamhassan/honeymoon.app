...
export default function LoggedInNav({ notifCount = 3 }) {
  ...
  return (
    <>
      <nav className="bg-[#174a37] h-[68px] flex items-center px-4 sm:px-6 lg:px-10 justify-between sticky top-0 z-50">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          <Link href="/notifications" className="relative text-white/70 hover:text-white transition-colors">
            ...
            {notifCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-[#b89b6b] text-white text-[10px] rounded-full w-4 h-4 flex items-center justify-center">{notifCount}</span>
            )}
          </Link>
          <div className="relative">
            <button onClick={() => setProfileOpen(!profileOpen)} className="flex items-center gap-2 text-white hover:text-white/80 transition-colors">
              <div className="w-8 h-8 bg-[#b89b6b] rounded-full flex items-center justify-center text-white text-sm font-medium">JH</div>
            ...
            {profileOpen && (
              <>
                ...
                <div className="absolute right-0 top-full mt-2 bg-white rounded-xl shadow-xl border border-[rgba(184,154,105,0.2)] py-2 w-44 z-50">
                  {[
                    ...
                    <Link key={item.href} href={item.href} onClick={() => setProfileOpen(false)}
                      className="block px-4 py-2.5 text-sm text-[#1a1a1a] hover:bg-[#f4ebd0] hover:text-[#174a37] transition-colors">
                      {item.label}
                    </Link>
                  ))}
                  <div className="border-t border-[rgba(184,154,105,0.1)] mt-1 pt-1">
          ...
          {/* Mobile hamburger */}
          <button onClick={() => setMobileOpen(!mobileOpen)} className="lg:hidden flex flex-col gap-1.5 p-1 ml-1" aria-label="Toggle menu">
      ...
      {mobileOpen && (
        <div className="bg-[#174a37] border-t border-white/10 py-2 shadow-lg lg:hidden sticky top-[68px] z-40">
          {navLinks.map(l => (
            <Link key={l.label} href={l.href} onClick={() => setMobileOpen(false)}
              className={`block px-6 py-3 text-[14px] uppercase tracking-wide transition-colors hover:bg-white/10 ${
                pathname === l.href ? 'text-[#b89b6b] font-bold' : 'text-white/80'
              }`}>
              {l.label}
            </Link>
          ))}
