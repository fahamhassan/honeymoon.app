  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: 'rgba(0,0,0,0.5)' }}>
      <div className="bg-white rounded-2xl w-full max-w-[500px] p-8 shadow-2xl">
        <h2 className="font-baskerville text-[26px] text-[#b89b6b] mb-5">Report a Booking Issue</h2>
        <div className="flex flex-col gap-4">
          ...
          <div>
            ...
            <textarea value={form.description} onChange={e => setForm(p=>({...p,description:e.target.value}))}
              ...
              className="w-full border border-[#d4d4d4] rounded-lg px-4 py-3 text-sm outline-none focus:border-[#b89b6b] resize-none transition-colors" />
          </div>
        </div>
        <div className="flex gap-3 mt-6">
          <button onClick={onSubmit} className="flex-1 bg-red-500 text-white font-medium py-3 rounded-xl hover:bg-red-600 transition-colors">Submit Report</button>
    ...
    <div className="min-h-screen bg-[#f4ebd0] font-sans flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-8 py-10 w-full">
        <div className="flex items-center justify-between mb-8">
          <h1 className="font-baskerville text-[40px] text-[#b89b6b]">Reported Bookings</h1>
          <button onClick={() => setShowModal(true)} className="bg-red-500 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-red-600 transition-colors">
            + New Report
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-[0_2px_15px_rgba(0,0,0,0.06)] overflow-hidden">
          <div className="divide-y divide-[rgba(184,154,105,0.08)]">
            {displayReports.map(r => (
              <div key={r.id} className="flex items-center gap-5 p-5 hover:bg-[#faf7f0] transition-colors">
                ...
                <span className={`text-xs font-medium px-3 py-1.5 rounded-full ${r.status==='Resolved'?'bg-green-100 text-green-700':'bg-amber-100 text-amber-700'}`}>{r.status}</span>
              </div>
            ))}
          </div>
        </div>
