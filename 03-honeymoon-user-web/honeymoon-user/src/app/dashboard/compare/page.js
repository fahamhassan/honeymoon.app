...
export default function ComparePage() {
  const { items: vendors, loading, total, hasMore, nextPage} = usePaginated(UserService.getVendors, {});
  ...
  return (
    ...
    <div className="max-w-[1200px]">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-baskerville text-[36px] text-[#1a1a1a]">Compare Vendors</h1>
          <p className="text-black/40 text-sm mt-1">Side-by-side comparison to make the right choice</p>
        </div>
        <Link href="/dashboard/vendors" className="border border-[rgba(184,154,105,0.3)] text-[#b89b6b] text-sm font-medium px-5 py-2.5 rounded-[10px] hover:bg-[#f4ebd0] transition-colors">
          + Add Vendor
        </Link>
      </div>
      ...
      <div className="flex gap-3 mb-8 flex-wrap">
        {vendors.map((v, i) => (
          <button key={v.id} onClick={() => setSelected(p => p.includes(i) ? p.filter(x => x !== i) : p.length < 3 ? [...p, i] : p)}
            className={`flex items-center gap-3 px-4 py-2.5 rounded-xl border transition-all text-sm font-medium ${
              selected.includes(i) ? 'border-[#174a37] bg-[#f4ebd0] text-[#174a37]' : 'border-[rgba(184,154,105,0.2)] bg-white text-black/50 hover:border-[#b89b6b]'
            }`}>
      ...
      {displayed.length < 2 ? (
        <div className="bg-white rounded-2xl border border-[rgba(184,154,105,0.2)] p-20 text-center">
        ...
        <div className="bg-white rounded-2xl border border-[rgba(184,154,105,0.2)] shadow-[0_0_30px_rgba(0,0,0,0.04)] overflow-hidden">
          {/* Header row */}
          <div className={`grid border-b border-[rgba(184,154,105,0.1)]`}
            style={{ gridTemplateColumns: `200px repeat(${displayed.length}, 1fr)` }}>
            <div className="p-6" />
            {displayed.map(v => (
              <div key={v.id} className="p-6 border-l border-[rgba(184,154,105,0.1)]">
                <div className="w-full h-36 rounded-xl overflow-hidden mb-4">
                  <img src={v.img} alt={v.name} className="w-full h-full object-cover" />
                </div>
                <span className="text-[#b89b6b] text-[11px] uppercase tracking-wider">{v.cat}</span>
          ...
          {/* Metrics */}
          {rows.map(row => (
            <div key={row.key} className={`grid border-b border-[rgba(184,154,105,0.06)] hover:bg-[#faf7f0] transition-colors`}
              style={{ gridTemplateColumns: `200px repeat(${displayed.length}, 1fr)` }}>
              ...
              {displayed.map(v => {
                ...
                return (
                  <div key={v.id} className="p-4 px-6 border-l border-[rgba(184,154,105,0.06)] flex items-center">
          ...
          {/* Features */}
          <div className={`grid bg-[#f9f6ef] border-b border-[rgba(184,154,105,0.1)]`}
            style={{ gridTemplateColumns: `200px repeat(${displayed.length}, 1fr)` }}>
            <div className="p-4 px-6"><span className="text-xs font-medium text-black/40 uppercase tracking-wider">Included</span></div>
            {displayed.map(v => <div key={v.id} className="p-4 border-l border-[rgba(184,154,105,0.06)]" />)}
          </div>
          {featureRows.map(f => (
            <div key={f.key} className={`grid border-b border-[rgba(184,154,105,0.06)] hover:bg-[#faf7f0] transition-colors`}
              style={{ gridTemplateColumns: `200px repeat(${displayed.length}, 1fr)` }}>
              <div className="p-4 px-6 flex items-center">
            ...
            {displayed.map(v => (
              <div key={v.id} className="px-3 flex flex-col gap-2">
                ...
                <Link href="/dashboard/chat"
                  className="w-full border border-[rgba(184,154,105,0.4)] text-[#b89b6b] text-xs font-medium py-2.5 rounded-[8px] hover:bg-[#f4ebd0] transition-colors text-center">
                  Message
                </Link>
              </div>
            ))}
