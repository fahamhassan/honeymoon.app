...
export default function SearchPage() {
  ...
  return (
    ...
    <div className="max-w-[900px]">
      <h1 className="font-baskerville text-[36px] text-[#1a1a1a] mb-6">Search</h1>

      {/* Search input */}
      <div className="bg-white border border-[rgba(184,154,105,0.3)] rounded-2xl px-6 py-4 flex items-center gap-4 mb-6 shadow-[0_0_30px_rgba(0,0,0,0.04)] focus-within:border-[#b89b6b] transition-colors">
        <span className="text-black/30 text-xl">🔍</span>
        <input value={query} onChange={e => setQuery(e.target.value)}
          placeholder="Search vendors, bookings, messages..."
          className="flex-1 text-[#1a1a1a] text-base outline-none bg-transparent placeholder-black/30"
          autoFocus />
        {query && <button onClick={() => setQuery('')} className="text-black/30 hover:text-black/60 text-lg transition-colors">✕</button>}
      </div>
        ...
        {filters.map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-5 py-2 rounded-full text-sm font-medium transition-colors ${
              filter === f ? 'bg-[#174a37] text-white' : 'bg-white border border-[rgba(184,154,105,0.2)] text-black/50 hover:border-[#b89b6b]'
            }`}>
            {f}
          </button>
        ))}
      ...
      {query === '' ? (
            ...
            <div className="flex gap-2 justify-center flex-wrap">
              {['Al Habtoor', 'Photography', 'Dubai Venues'].map(s => (
                <button key={s} onClick={() => setQuery(s)}
                  className="text-sm text-[#b89b6b] bg-[#f4ebd0] px-4 py-1.5 rounded-full hover:bg-[#e8dfc5] transition-colors">
                  {s}
                </button>
              ))}
            </div>
        ...
        <div className="text-center py-20">
          ...
          <Link href="/dashboard/vendors" className="inline-block mt-4 text-[#b89b6b] hover:underline text-sm">Browse all vendors →</Link>
        </div>
      ) : (
        <div>
          <p className="text-xs text-black/40 uppercase tracking-wider mb-4">{results.length} result{results.length !== 1 ? 's' : ''}</p>
          <div className="bg-white rounded-2xl border border-[rgba(184,154,105,0.2)] shadow-[0_0_30px_rgba(0,0,0,0.04)] divide-y divide-[rgba(184,154,105,0.08)] overflow-hidden">
            {results.map(r => (
              <Link key={r.id} href={r.href}
                className="flex items-center gap-5 p-5 hover:bg-[#faf7f0] transition-colors group">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full ${
                      r.type === 'vendor' ? 'bg-[#f4ebd0] text-[#b89b6b]' : 'bg-blue-50 text-blue-600'
                    }`}>{r.type}</span>
                  </div>
                ...
                <span className="text-black/20 group-hover:text-[#b89b6b] transition-colors text-lg">→</span>
              </Link>
            ))}
          </div>
        </div>
