...
export default function InvitePage() {
  ...
  return (
    <div className="max-w-[800px]">
      <div className="mb-8">
        <h1 className="font-baskerville text-[36px] text-[#1a1a1a]">Invite & Share</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 flex flex-col gap-5">
          <div className="bg-white rounded-2xl border border-[rgba(184,154,105,0.2)] p-6 shadow-[0_0_30px_rgba(0,0,0,0.04)]">
            <h2 className="font-baskerville text-[22px] text-[#1a1a1a] mb-5">Invite by Email</h2>
            <div className="flex gap-3 mb-4">
              <input value={email} onChange={e => setEmail(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && invite()}
                placeholder="name@example.com"
                className="flex-1 border border-[rgba(184,154,105,0.3)] rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#b89b6b] transition-colors" />
              <select value={role} onChange={e => setRole(e.target.value)}
                className="border border-[rgba(184,154,105,0.3)] rounded-xl px-3 py-2.5 text-sm bg-white outline-none focus:border-[#b89b6b] transition-colors">
                {roles.map(r => <option key={r}>{r}</option>)}
              </select>
            </div>
            <button onClick={invite}
              className={`w-full text-sm font-medium py-3 rounded-xl transition-colors ${
                email ? 'bg-[#174a37] text-white hover:bg-[#1a5c45]' : 'bg-[#f4ebd0] text-black/30 cursor-not-allowed'
              }`}>
          ...
          <div className="bg-white rounded-2xl border border-[rgba(184,154,105,0.2)] p-6 shadow-[0_0_30px_rgba(0,0,0,0.04)]">
            <h2 className="font-baskerville text-[22px] text-[#1a1a1a] mb-2">Share Link</h2>
            <p className="text-black/40 text-sm mb-4">Anyone with this link can join as a Viewer</p>
            <div className="flex gap-3">
              <div className="flex-1 bg-[#f9f6ef] border border-[rgba(184,154,105,0.2)] rounded-xl px-4 py-2.5 text-sm text-black/40 truncate">
                {link || 'Generate a shareable link...'}
              </div>
              <button onClick={copyLink}
                className={`px-5 py-2.5 text-sm font-medium rounded-xl transition-colors flex-shrink-0 ${
      ...
      {/* Team members */}
      <div className="bg-white rounded-2xl border border-[rgba(184,154,105,0.2)] shadow-[0_0_30px_rgba(0,0,0,0.04)] mt-6">
        <div className="flex items-center justify-between p-6 border-b border-[rgba(184,154,105,0.1)]">
          <h2 className="font-baskerville text-[22px] text-[#1a1a1a]">Wedding Team</h2>
        ...
        <div className="divide-y divide-[rgba(184,154,105,0.08)]">
          {members.map((m, i) => (
            <div key={i} className="flex items-center gap-4 p-5 hover:bg-[#faf7f0] transition-colors">
              <div className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0 bg-[#174a37] flex items-center justify-center">
                {m.img
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-[#1a1a1a] flex items-center gap-2">
                  {m.name}
                  {m.you && <span className="text-[10px] text-[#b89b6b] bg-[#f4ebd0] px-2 py-0.5 rounded-full">You</span>}
                </p>
                <p className="text-xs text-black/40 mt-0.5">{m.email}</p>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-xs font-medium text-[#174a37] bg-[#f4ebd0] px-3 py-1 rounded-full">{m.role}</span>
                {!m.you && (
                  <button onClick={() => removeRole(i)} className="text-black/20 hover:text-red-400 transition-colors text-lg">×</button>
                )}
              </div>
            </div>
          ))}
