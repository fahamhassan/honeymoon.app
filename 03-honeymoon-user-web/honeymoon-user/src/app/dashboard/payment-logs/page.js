  ...
  return (
    <div className="min-h-screen bg-[#f4ebd0] font-sans flex flex-col">
      ...
      <main className="flex-1 max-w-5xl mx-auto px-8 py-10 w-full">
        <h1 className="font-baskerville text-[40px] text-[#b89b6b] mb-4">Payment Logs</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          {[['Total Paid',`AED ${totalPaid.toLocaleString()}`,'✓'],['Upcoming',`AED ${source.filter(l=>l.status==='Upcoming'||l.status==='pending').reduce((s,l)=>s+(typeof l.amount==='number'?l.amount:parseInt(String(l.amount).replace(/[^0-9]/g,''))||0),0).toLocaleString()}`,'⏰'],['Scheduled',`AED ${source.filter(l=>l.status==='Scheduled').reduce((s,l)=>s+(typeof l.amount==='number'?l.amount:0),0).toLocaleString()}`,'📅']].map(([l,v,icon]) => (
            <div key={l} className="bg-white rounded-2xl p-5 shadow-[0_2px_15px_rgba(0,0,0,0.06)]">
              <p className="text-black/40 text-xs uppercase tracking-wider">{icon} {l}</p>
              <p className="font-baskerville text-2xl text-[#174a37] mt-1">{v}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-2 mb-5">
          {['All','Paid','Upcoming','Scheduled'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-4 py-2 rounded-full text-xs font-medium transition-all ${filter===f?'bg-[#174a37] text-white':'bg-white border border-[rgba(184,154,105,0.3)] text-gray-600 hover:border-[#b89b6b]'}`}>
              {f}
            </button>
          ))}
        </div>
            ...
            <thead>
              <tr className="border-b border-gray-100">
                {['Transaction','Vendor','Type','Amount','Date','Method','Status'].map(h => (
                  <th key={h} className="text-left py-3.5 px-5 text-black/40 font-medium text-xs uppercase tracking-wide">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map(l => (
                <tr key={l.id} className="border-b border-[rgba(184,154,105,0.06)] hover:bg-[#fafaf8] transition-colors">
                  <td className="py-4 px-5 font-mono text-xs text-black/40">{l.id}</td>
                  <td className="py-4 px-5 font-medium text-[#1a1a1a]">{l.vendor}</td>
                  <td className="py-4 px-5 text-black/50 text-xs">{l.type}</td>
                  <td className="py-4 px-5 font-baskerville text-lg text-[#174a37]">{l.amount}</td>
                  <td className="py-4 px-5 text-black/40 text-xs">{l.date}</td>
                  <td className="py-4 px-5 text-black/50 text-xs">{l.method}</td>
                  <td className="py-4 px-5"><StatusBadge s={l.status} /></td>
                </tr>
              ))}
            </tbody>
          </table>
