...
const cats = [
  ...
  { name: 'Catering', pct: 25, color: '#b89b6b' },
  ...
  return (
    <div className="min-h-screen bg-[#f4ebd0] font-sans flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-8 py-10 w-full">
        <h1 className="font-baskerville text-[40px] text-[#b89b6b] mb-8">Budget Estimation</h1>

        <div className="bg-white rounded-2xl p-8 shadow-[0_2px_15px_rgba(0,0,0,0.06)] max-w-[900px]">
            ...
            <div className="flex-1">
              <div className="mb-5">
                ...
                <div className="relative">
                  <select value={location} onChange={e => setLocation(e.target.value)}
                    className="w-full border border-[#d4d4d4] rounded-lg px-4 py-3 text-sm outline-none focus:border-[#b89b6b] bg-white appearance-none transition-colors">
              ...
              <div className="mb-6">
                ...
                <input type="number" value={guests} onChange={e => setGuests(e.target.value)}
                  ...
                  className="w-full border border-[#d4d4d4] rounded-lg px-4 py-3 text-sm outline-none focus:border-[#b89b6b] transition-colors" />
              </div>
              <button onClick={() => location && guests && setEstimated(true)}
                className={`px-8 py-3 rounded-xl font-medium text-sm transition-colors ${location && guests ? 'bg-[#b89b6b] text-white hover:bg-[#a08860]' : 'bg-[#e8e0d0] text-black/30 cursor-not-allowed'}`}>
                Generate Estimation
              </button>
            </div>

            {/* Right: AI estimation result */}
            <div className="flex-1 bg-[#f9f6ef] rounded-2xl p-6 border border-[rgba(184,154,105,0.2)]">
              {estimated ? (
                <>
                  ...
                  <div className="flex flex-col gap-2 mb-5">
                    {cats.map(c => (
                      <div key={c.name} className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-medium text-[#1a1a1a]">AED {Math.round(totalBudget * c.pct / 100).toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                  <button onClick={() => setSaved(true)}
                    className={`w-full py-3 rounded-xl font-medium text-sm transition-colors ${saved ? 'bg-green-600 text-white' : 'bg-[#b89b6b] text-white hover:bg-[#a08860]'}`}>
