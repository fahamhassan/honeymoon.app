    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:flex flex-1 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div>
            <h2 className="font-baskerville text-[48px] leading-[56px] text-white capitalize mb-4">Verify your identity</h2>
        ...
        <div className="w-full max-w-[400px]">
          {verified ? (
            <div className="text-center">
              ...
              <h1 className="font-baskerville text-[36px] text-[#1a1a1a] mb-2">Verified!</h1>
            ...
            <>
              <h1 className="font-baskerville text-[36px] text-[#1a1a1a] mb-2">Enter OTP</h1>
              <p className="text-black/40 mb-2 text-sm">We sent a 6-digit code to <span className="text-[#1a1a1a] font-medium">r*****@gmail.com</span></p>
              ...
              <div className="flex gap-3 mb-8 justify-center">
                {otp.map((digit, i) => (
                  <input key={i} ref={el => inputs.current[i] = el}
                    ...
                    className={`w-12 h-14 text-center text-[24px] font-baskerville border-2 rounded-xl outline-none transition-all ${
                      digit ? 'border-[#174a37] bg-[#f4ebd0] text-[#174a37]' : 'border-[rgba(184,154,105,0.3)] focus:border-[#b89b6b]'
                    }`} />
                ))}
              </div>
              ...
              <button onClick={handleVerify}
                className={`w-full text-sm font-medium py-3.5 rounded-xl transition-colors mb-4 ${
                  otp.join('').length === 6 ? 'bg-[#174a37] text-white hover:bg-[#1a5c45]' : 'bg-[#f4ebd0] text-black/30 cursor-not-allowed'
                }`}>
                Verify OTP
              </button>
              ...
              <p className="text-center text-sm text-black/40">
                Didn't receive it?{' '}
                {timer === 0 ? (
                  <button onClick={() => { setResent(true); setTimer(59); }} className="text-[#b89b6b] font-medium hover:underline">Resend code</button>
                ) : (
              ...
              <div className="mt-8 text-center">
                <Link href="/login" className="text-sm text-black/40 hover:text-[#b89b6b] transition-colors">← Back to login</Link>
              </div>
            </>
          )}
        </div>
