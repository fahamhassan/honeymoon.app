  ...
  return (
    ...
    <div className="min-h-screen bg-[#f4ebd0] font-sans flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-8 py-10 w-full">
        <h1 className="font-baskerville text-[40px] text-[#b89b6b] mb-8">Meeting Requests</h1>
        <div className="flex gap-2 mb-6">
          {['All','Confirmed','Pending','Cancelled'].map(s => (
            <button key={s} onClick={() => setFilter(s)}
              className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${filter===s?'bg-[#174a37] text-white':'bg-white border border-[rgba(184,154,105,0.3)] text-black/60 hover:border-[#b89b6b]'}`}>
              {s}
            </button>
          ))}
        </div>
        <div className="bg-white rounded-2xl shadow-[0_2px_15px_rgba(0,0,0,0.06)] overflow-hidden">
          <div className="divide-y divide-[rgba(184,154,105,0.08)]">
            {filtered.map(m => (
              <div key={m.id} className="flex items-center gap-5 p-5 hover:bg-[#faf7f0] transition-colors">
                <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0">
                  <img src={m.img} alt={m.vendor} className="w-full h-full object-cover" />
                </div>
                <div className="flex-1">
                  <p className="font-medium text-[#1a1a1a]">{m.vendor}</p>
                  <p className="text-black/40 text-sm">{m.type}</p>
                  <p className="text-[#174a37] text-sm mt-0.5">📅 {m.date} · ⏰ {m.time}</p>
                </div>
                <span className={`text-xs font-medium px-3 py-1.5 rounded-full ${statusColors[m.status]}`}>{m.status}</span>
              </div>
            ))}
          </div>
        </div>
