    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:flex flex-1 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-between p-12">
          ...
          <div>
            <h2 className="font-baskerville text-[48px] leading-[56px] text-white capitalize mb-4">Create a new password</h2>
      ...
      <div className="flex-1 lg:max-w-[520px] flex items-center justify-center p-8 bg-white">
        <div className="w-full max-w-[400px]">
          {done ? (
            ...
            <>
              <h1 className="font-baskerville text-[36px] text-[#1a1a1a] mb-1">Reset Password</h1>
                      ...
                      <div className="flex gap-1 flex-1">
                        {['weak', 'medium', 'strong'].map((s, i) => (
                          <div key={s} className={`h-1 flex-1 rounded-full ${
                            ...
                            (strength === 'strong') ? 'bg-green-400' : 'bg-[#f4ebd0]'
                          }`} />
                        ))}
                      </div>
                ...
                <div>
                  ...
                  <input type="password" value={form.confirm} onChange={e => setForm(p => ({ ...p, confirm: e.target.value }))}
                    ...
                    className={`w-full border rounded-xl px-4 py-3 text-sm outline-none transition-all ${
                      form.confirm ? (match ? 'border-green-400 focus:border-green-400' : 'border-red-300 focus:border-red-400') : 'border-[rgba(184,154,105,0.3)] focus:border-[#b89b6b]'
                    }`} />
                  {form.confirm && !match && <p className="text-xs text-red-400 mt-1">Passwords don't match</p>}
                </div>
                <button onClick={() => match && setDone(true)}
                  className={`w-full text-sm font-medium py-3.5 rounded-xl transition-colors ${match ? 'bg-[#174a37] text-white hover:bg-[#1a5c45]' : 'bg-[#f4ebd0] text-black/30 cursor-not-allowed'}`}>
                  Set New Password
                </button>
              </div>
            </>
