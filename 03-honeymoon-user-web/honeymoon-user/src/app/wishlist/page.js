    ...
    <div className="min-h-screen bg-[#f4ebd0] font-sans flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-8 py-10 w-full">
        <div className="flex items-center justify-between mb-8">
          <h1 className="font-baskerville text-[40px] text-[#b89b6b]">Wishlist</h1>
          <div className="flex items-center gap-3">
            <div className="bg-white border border-[rgba(184,154,105,0.3)] rounded-lg px-4 py-2.5 flex items-center gap-2 w-52">
              <span className="text-black/30">🔍</span>
              <input value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Search" className="bg-transparent text-sm outline-none flex-1 text-black/60 placeholder-black/30" />
            </div>
            <button className="w-10 h-10 border border-[rgba(184,154,105,0.3)] rounded-lg flex items-center justify-center text-black/40 hover:text-[#b89b6b] hover:border-[#b89b6b] transition-colors">
              ⊞
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map(v => (
            <div key={v.id} className="bg-white rounded-2xl overflow-hidden shadow-[0_2px_15px_rgba(0,0,0,0.06)] hover:shadow-[0_4px_25px_rgba(0,0,0,0.1)] transition-shadow group">
              ...
              <div className="p-4">
                <h3 className="font-medium text-[#1a1a1a] text-base">{v.name}</h3>
                <div className="flex items-center justify-between mt-2">
                  <p className="text-[#b89b6b] text-sm flex items-center gap-1">
                    <span className="text-black/40">👤</span> {v.vendor}
                  </p>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-20">
            ...
            <Link href="/vendors" className="inline-block mt-4 text-[#b89b6b] hover:underline">Browse Vendors →</Link>
          </div>
        )}

        <div className="flex items-center justify-between mt-10">
          <p className="text-sm text-black/50">Showing 1 To {filtered.length} From {items.length} Entries</p>
          <div className="flex items-center gap-2">
            <button className="px-3 py-1.5 text-sm border border-[rgba(184,154,105,0.3)] rounded-lg text-black/50 hover:border-[#b89b6b] transition-colors">← Previous</button>
            {[1,2,3].map(p => (
              <button key={p} className={`w-8 h-8 rounded-lg text-sm ${p===1?'bg-[#b89b6b] text-white':'text-black/50 border border-[rgba(184,154,105,0.3)] hover:border-[#b89b6b]'} transition-colors`}>{p}</button>
            ))}
            <button className="px-3 py-1.5 text-sm border border-[rgba(184,154,105,0.3)] rounded-lg text-black/50 hover:border-[#b89b6b] transition-colors">Next →</button>
          </div>
        </div>
      </main>
      <Footer />
